# Lab-Sidecar Artifact Protocol

This document describes the current public-alpha artifact contract for the
local CLI path. It is a compatibility target for cautious alpha development,
not a promise of permanent schema stability. It describes current
implementation behavior and does not define a hosted service, remote runner,
Web UI, FastAPI app, or general multi-agent framework.
...
MCP/V2 integrations are optional local adapters over these same task artifacts.
They are not the primary product surface.

本协议只定义当前本地文件约定，目标是简单、可恢复、可迁移，不做企业级平台设计。
...
.lab-sidecar/
  ...
  tasks/
    task_20260531_143000_a1b2c3/
      ...
      intelligence/
  comparisons/
    comparison_20260624_120000_a1b2c3/
...
Explicit specs are compatible with both the legacy single-spec YAML object and
the multi-spec `figures:` YAML shape documented in
[figure-specs.md](figure-specs.md). The deterministic renderer supports
`line`, `bar`, and `box`.
...
Reports are deterministic fragments from recorded artifacts. They are not AI
analysis, statistical-significance claims, or paper-ready conclusions.
...
Slides are static editable PPTX drafts. They do not include animation, video,
or autonomous research conclusions.
...
Traceability records task id, source references, generated artifact hashes and
sizes, metric lineage, figure lineage, report claim traces, slide evidence,
reproduce metadata pointers, package traceability evidence, and omission notes.
It does not embed full logs, full metric rows, report bodies, PPTX contents,
worker prompt/response bodies, raw sources, SQLite, or unrelated workspace
files.
...
保存复现所需的命令、环境和依赖引用，例如 `command.txt`、`env.json`、`git.json`、`dependencies.json`。
...
`labsidecar package <task_id> --output <dir>` creates a single-task result or
diagnostic package from an allowlist. Current package metadata files:
...
Generated metrics, figures, report, slides, reproduce metadata, and
`provenance/traceability.json` are copied when present. Full logs, raw sources,
SQLite, worker prompt/response bodies, worker audit files, sandbox files, and
unrelated workspace files are omitted by default.
...
paths, categories, descriptions, sizes, and SHA-256 hashes when applicable.
`artifact-index.sha256` stores a checksum line for `artifact-index.json`.

`labsidecar package-verify <package_dir>` checks:

- `artifact-index.sha256` exists and matches `artifact-index.json`.
- Included files and package metadata files exist.
- Recorded sizes and SHA-256 hashes match package contents.
- Files not listed in `artifact-index.json` are reported as unexpected.

## Validation Contract

`labsidecar validate <task_id>` reads existing task artifacts and does not
generate new artifacts. It checks task manifest parseability, log presence,
metrics consistency, figure summary/image presence, report summary/fragment
presence, slide summary/PPTX presence, traceability consistency, and optional
`--require package-ready` readiness. A partial task may produce warnings;
inconsistent or missing required artifacts produce failures.

`labsidecar validate-comparison <comparison_id>` reads existing saved
comparison artifacts and does not generate new artifacts. It checks comparison
manifest parseability, comparison summary boundedness, comparison table
consistency, optional figure/report presence, comparison traceability, and
source metric artifact digests. `--require package-ready` is the stricter full
comparison result-package readiness check: it requires figures, report,
traceability, and registered artifact presence. `package-comparison` can still
create a metadata/table package when optional figure or report artifacts have
not been generated; those optional files are listed as unavailable.

## Comparison Artifact Contract

Saved comparison records live under
`.lab-sidecar/comparisons/<comparison_id>/`. They are derived local artifacts,
not task records and not command executions.
...
- `comparison-table.json`
- `figures/figure-summary.json`
- `figures/comparison_<metric>.png`
- `figures/comparison_<metric>.svg`
- `reports/comparison-report-fragment.md`
- `reports/comparison-report-summary.json`
- `provenance/traceability.json`

The first implementation compares 2-5 already-collected local tasks using each
task's final row from `metrics/normalized_metrics.csv`. It compares shared
numeric fields only and excludes metadata/identity/checkpoint fields such as
`source_file`, `source_path`, `file`, `path`, `epoch`, `step`, `iter`,
`iteration`, `checkpoint`, `ckpt`, `timestamp`, `seed`, `trial`, and `run_id`.

`comparison-summary.json` records source task ids, task metadata, source metric
paths, source metric hashes and sizes, selected row numbers, common numeric
fields, skipped fields, bounded metric values, descriptive `best_by_metric`
entries, warnings, omissions, and evidence paths. It does not embed complete
source metric rows or logs.

Comparison figures and reports are deterministic. Report text must include the
non-claim note: "This comparison is descriptive only; no statistical
significance or model superiority is inferred."

## Omitted By Default

The local artifact contract deliberately omits bulky or sensitive bodies from
shareable packages, traceability, and agent-facing responses by default:

- Full stdout/stderr logs.
- Full normalized metric rows.
- Full report bodies in agent responses.
- PPTX contents in agent responses.
- Worker prompt/response bodies.
- Raw source files.
- Local SQLite indexes.
- Temporary sandbox files and unrelated workspace files.
