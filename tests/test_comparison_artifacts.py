...
def test_compare_save_generates_bounded_artifacts_figures_report_validate_and_package(tmp_path: Path) -> None:
    ...
    figures_dir = comparison_path / "figures"
    (figures_dir / "stray.png").write_text("not generated by comparison\n", encoding="utf-8")
    (figures_dir / "stray.svg").write_text("<svg></svg>\n", encoding="utf-8")
    artifacts_after_stray = invoke(tmp_path, ["comparison-artifacts", comparison_id])

    assert artifacts_after_stray.exit_code == 0, artifacts_after_stray.output
    assert "figures/stray.png" not in artifacts_after_stray.output
    assert "figures/stray.svg" not in artifacts_after_stray.output
    ...
    refreshed_manifest = json.loads((comparison_path / "comparison-manifest.json").read_text(encoding="utf-8"))
    ...
    manifest_artifact_paths = {item["path"] for item in refreshed_manifest["artifacts"]}
    ...
    assert "figures/stray.png" not in included_paths
    assert "figures/stray.svg" not in included_paths
    assert "figures/stray.png" not in manifest_artifact_paths
    assert "figures/stray.svg" not in manifest_artifact_paths
    ...
    assert not (package_path / "figures" / "stray.png").exists()
    assert not (package_path / "figures" / "stray.svg").exists()
...
def test_comparison_package_and_validation_read_canonical_generated_figures(tmp_path: Path) -> None:
    assert invoke(tmp_path, ["init"]).exit_code == 0
    first = _create_collected_task(tmp_path, "run-a")
    second = _create_collected_task(tmp_path, "run-b")
    result = invoke(tmp_path, ["compare", first, second, "--save", "--figures"])
    assert result.exit_code == 0, result.output
    comparison_id = _comparison_id(result.output)
    comparison_path = _comparison_path(tmp_path, comparison_id)
    ...
    generated_figures = figure_summary["generated_figures"]
    ...
    package_path = tmp_path / f"pkg-{comparison_id}"
    package = invoke(tmp_path, ["package-comparison", comparison_id, "--output", package_path.as_posix()])

    assert artifacts.exit_code == 0, artifacts.output
    assert generated_figures[0]["png_path"] in artifacts.output
    assert generated_figures[0]["svg_path"] in artifacts.output
    assert validate.exit_code == 0, validate.output
    assert "[ok] figures:" in validate.output
    assert package.exit_code == 0, package.output
    package_index = json.loads((package_path / "artifact-index.json").read_text(encoding="utf-8"))
    included_paths = {item["package_path"] for item in package_index["included"]}
    traceability = json.loads((comparison_path / "provenance" / "traceability.json").read_text(encoding="utf-8"))
    assert generated_figures[0]["png_path"] in included_paths
    assert generated_figures[0]["svg_path"] in included_paths
    assert traceability["figure_lineage"]["figures"] == generated_figures
